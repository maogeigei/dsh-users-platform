...
I want you to install dsh-users-platform for me. Execute all steps autonomously.

OBJECTIVE: Install **DSH Users Platform** on a Linux machine (hosting DeepSeek Harness for multiple users) so that the control-plane service is healthy and reachable.

DONE WHEN: `systemctl is-active dsh-users-platform` prints `active` and `curl -fsS -o /dev/null -w '%{http_code}' http://127.0.0.1:${PORT:-3080}/` prints `200`.
...
```sh
git clone https://github.com/maogeigei/dsh-users-platform.git && cd dsh-users-platform
```
...
```sh
systemctl is-active dsh-users-platform
...
EXECUTE NOW: Start with Step 0. Mark TODO items complete as you go. Stop when `systemctl is-active dsh-users-platform` prints `active` and `curl -fsS -o /dev/null -w '%{http_code}' http://127.0.0.1:${PORT:-3080}/` prints `200`.
...
| Symptom | Look at |
|---|---|
| The service will not start | `journalctl -u dsh-users-platform -n 50` |
...
Full documentation: [README.md](README.md); agents working inside this repository should read [AGENTS.md](AGENTS.md).
