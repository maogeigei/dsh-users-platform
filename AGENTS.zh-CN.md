...
2. **改完必须过 `npm run verify`**（它含 `npm run build` + 单元测试 + 5 个静态校验脚本）。只跑 `typecheck` 不够。
3. **关键决策都写成了纯函数**（崩溃策略、路径围栏、patch 渲染、安装路径解析…）——它们不依赖进程与网络，
   ⇒ 改这类逻辑时**请同时补 `test/` 或 `scripts/verify-*.mjs` 断言**，而不是只改实现。回归网是这类代码唯一的保险。
4. **改了注入脚本（`assets/inject/`）** 要跑 `node scripts/verify-inject.cjs lib/supervisor/proxy.js`（`npm test` 已含）。
...
- [ ] `npm run verify` 全绿
- [ ] 新增/修改的行为有对应断言（单元测试或 `scripts/verify-*.mjs`）
