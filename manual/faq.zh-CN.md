...
`DSH_USERS_PLATFORM_DSH_BIN` 要写**绝对路径**（`which dsh` 的结果）—— systemd 的 PATH 很精简，写 `dsh` 通常找不到。
</details>
...
按序查：`systemctl status dsh-users-platform` → `journalctl -u dsh-users-platform -n 100` → 管理台「服务管理」里实例是否在跑 → nginx `error_log`。反复重启通常是某插件与当前 DSH 版本不兼容，在「功能管理」里禁用它。
</details>
