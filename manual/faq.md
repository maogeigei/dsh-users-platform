...
`DSH_USERS_PLATFORM_DSH_BIN` must be an **absolute path** (the output of `which dsh`) — systemd's PATH is minimal, so plain `dsh` usually is not found.
</details>
...
Check in order: `systemctl status dsh-users-platform` → `journalctl -u dsh-users-platform -n 100` → whether the instance is running under "Service management" in the admin console → the nginx `error_log`. Repeated restarts usually mean a plugin is incompatible with the current DSH version; disable it under "Feature management".
</details>
