...
const DB_PATH = '/var/lib/dsh-users-platform/dsh-users-platform.db'
const PROFILE = 'web' // 当前唯一 profile
const MARK = '# dsh-users-platform role patch'
// --force：已由本脚本管理但内容落后（缺新版禁用项）时，整体升级为当前块。
const FORCE = process.argv.includes('--force')
const DISABLE_MODELS_BLOCK = [
  '# dsh-users-platform role patch: 普通用户隐藏模型分区 + 收归核心插件开关（/ 87）',
...
const ADMIN_MODELS_BLOCK = [
  '# dsh-users-platform role patch: admin 仅隐藏「模型」分区',
