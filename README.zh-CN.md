...
**全部代码与文档由 AI 生成**（DeepSeek **V4 / V4.1 flash**）—— 见 [AI 生成](manual/project.zh-CN.md#ai-生成)。
...
| **[manual/project.zh-CN.md](manual/project.zh-CN.md)** | 开发、贡献、版本、AI 生成 |
...
实例被回收后再打开，看到的是一个**会自动恢复**的过渡页（不是报错）：
...
- **限定单机部署** —— 不受支持的 `deployMode` **直接报错**，不再静默回退到单机后端。
...
以 **GNU Affero General Public License v3.0** 开源，全文见 [LICENSE](LICENSE)。

简言之：可自由使用、修改、分发；**若你修改后以网络服务的形式提供给别人使用，需向这些使用者提供修改后的源码**（AGPL-3.0 §13）。
