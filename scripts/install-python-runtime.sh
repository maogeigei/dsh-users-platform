...
if [ -x "$PYDIR/bin/python3" ]; then
  ...
  if [ -z "$TARBALL" ]; then
    for c in "/var/lib/dsh-users-platform/artifacts/cpython-$PY_VER.tar.gz" "$RT/cpython-$PY_VER.tar.gz"; do
      [ -f "$c" ] && TARBALL="$c" && break
    done
  fi
  if [ -z "$TARBALL" ] || [ ! -f "$TARBALL" ]; then
    echo "==> 本地无 tarball，联网下载（$PY_VER）"
    mkdir -p /var/lib/dsh-users-platform/artifacts
    TARBALL="/var/lib/dsh-users-platform/artifacts/cpython-$PY_VER.tar.gz"
