 ...
 *   ① /var/lib/dsh-users-platform/state/capabilities.json      —— 给平台 API / 门户（机读）
...
const STATE = process.env.DSH_STATE_DIR ?? '/var/lib/dsh-users-platform/state'
const OUT_JSON = join(STATE, 'capabilities.json')
const BUNDLED = process.env.DSH_BUNDLED_SKILL_DIR ?? '/var/lib/dsh-users-platform/bundled-skills'
const USERS = process.env.DSH_USERS_DIR ?? '/var/lib/dsh-users-platform/users'
...
const capabilities = {
  ...
  fileDelivery: {
    ...
    avoid: ['不要给 `/var/lib/dsh-users-platform/users/...` 这类服务器绝对路径', '不要给 127.0.0.1:<port> 链接（用户浏览器打不开）'],
...
description: 本 dsh 实例（托管在 dsh-users-platform 平台上）的**能力边界与交付约定**。开工前读它可省去自行探测；包含可读/可写路径、网络边界、可用工具版本、已注册技能清单、以及把文件交给用户的正确方式。
---
