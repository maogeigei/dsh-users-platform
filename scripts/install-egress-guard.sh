...
table ip dsh_egress {
	chain output {
		...
		#   <HOST_LAN_IP>  → 宿主内网卡（eth0，同样到达 nginx/面板）
		#   172.17.0.1     → docker0 网桥网关
		#   <SERVER_PUBLIC_IP>   → 公网 EIP（回环到本机 nginx/sshd）
		# 不影响：公网访问、云厂商内网 DNS、pip/npm 下载、
		#         实例自身监听端口、nginx 回源（root 发包 + 实例回包带 ack）
		meta skuid 100000-199999 ip daddr { 127.0.0.0/8, 172.17.0.1, <HOST_LAN_IP>, <SERVER_PUBLIC_IP> } \
