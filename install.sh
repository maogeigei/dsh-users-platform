#!/usr/bin/env bash
# ============================================================================
# dsh-users-platform —— 单机一键部署（模式 A：裸机 + systemd + nginx）
#
# 用法（在仓库根目录，需 root）：
...
DOMAIN=""
EMAIL=""
PORT="${DSH_USERS_PLATFORM_PORT:-3080}"
ADMIN_USER="admin"
ADMIN_PASS=""
DATA_ROOT="${DSH_USERS_PLATFORM_DATA_ROOT:-/var/lib/dsh-users-platform}"
ENV_FILE="/etc/dsh-users-platform.env"
UNIT_FILE="/etc/systemd/system/dsh-users-platform.service"
...
NGINX_CONF="$(detect_nginx_conf_dir)/dsh-users-platform.conf"
...
参数：
  ...
  --data-root <路径>   每用户数据根（默认 /var/lib/dsh-users-platform）。
...
write_env() {
  ...
  cat > "$ENV_FILE" <<EOF
# dsh-users-platform 环境文件（由 install.sh 生成，0600）
DSH_USERS_PLATFORM_PORT=$PORT
DSH_USERS_PLATFORM_DATA_ROOT=$DATA_ROOT
DSH_USERS_PLATFORM_DSH_BIN=$DSH_BIN
DSH_USERS_PLATFORM_ISOLATION_MODE=$ISOLATION_MODE
DSH_USERS_PLATFORM_BASE_UID=100000
DSH_USERS_PLATFORM_PORT_GUARD=$PORT_GUARD
DSH_USERS_PLATFORM_SECURE_COOKIES=$secure
DSH_USERS_PLATFORM_BASE_DOMAIN=$base_domain
DSH_USERS_PLATFORM_COOKIE_DOMAIN=$cookie_domain
DSH_USERS_PLATFORM_ENABLE_PATCH=true
DSH_USERS_PLATFORM_DEPLOY_MODE=local
EOF
  chmod 600 "$ENV_FILE"
  c_ok "  ✓ 已写入（0600）"
...
write_unit() {
  ...
  if [ "$DRY_RUN" = "1" ]; then
    ...
    cat > "$UNIT_FILE" <<EOF
[Unit]
Description=dsh-users-platform — 多租户 DSH 托管平台（控制面）
