 ...
 *
 * 解析顺序（命中即停）：
 *   1. env `DSH_PACKAGE_DIR` / `DSH_COMPAT_ROOT`（+ 本仓前缀别名 `DSH_USERS_PLATFORM_PACKAGE_DIR` /
 *      `DSH_USERS_PLATFORM_COMPAT_ROOT`）—— 显式覆盖，也是测试注入点，**无条件生效**；
 *   2. 平台配置的 dsh 可执行文件（`DSH_USERS_PLATFORM_DSH_BIN`）
 ...
 *
 * @module dsh-users-platform/web/dsh-install
 */
...
function rootFromConfiguredBin(): string | null {
  const bin = firstEnv(['DSH_USERS_PLATFORM_DSH_BIN'])
...
export function dshPackageRoot(): string {
  ...
  const envRoot = firstEnv(['DSH_PACKAGE_DIR', 'DSH_USERS_PLATFORM_PACKAGE_DIR', 'DSH_COMPAT_ROOT', 'DSH_USERS_PLATFORM_COMPAT_ROOT'])
  if (envRoot !== null) {
    cachedRoot = envRoot
    return envRoot
  }
...
export function piAiDataDir(): string {
  const envDir = firstEnv(['PI_AI_DATA_DIR', 'DSH_USERS_PLATFORM_PI_AI_DATA_DIR'])
