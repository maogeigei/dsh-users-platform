 ...
 *
 * @module dsh-users-platform/web/model-landing
 */
...
const markBegin = (route: string): string => `    # dsh-users-platform:model-route ${route} begin`
const markEnd = (route: string): string => `    # dsh-users-platform:model-route ${route} end`
...
export function reconcileSettings(
  ...
  for (let i = 0; i < lines.length; i++) {
    const b = /^[ \t]*# dsh-users-platform:model-route ([^\s]+) begin[ \t]*$/.exec(lines[i])
    if (b === null) {
    ...
    const route = b[1]
    const endRe = new RegExp(`^[ \\t]*# dsh-users-platform:model-route ${escapeRe(route)} end[ \\t]*$`)
    let end = -1
