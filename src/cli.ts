#!/usr/bin/env node
/**
 * Standalone orchestrator entry (`dsh-users-platform` bin).
 *
 * Subcommands:
 *   dsh-users-platform bootstrap-admin --username <u> --password <p>
 *   dsh-users-platform [server flags]
 * @module dsh-users-platform/cli
 */
...
const HELP = `dsh-users-platform — DSH server login orchestrator
...
bootstrap-admin options:
  ...
  --password <p>    Admin password (or DSH_USERS_PLATFORM_ADMIN_PASSWORD env).
...
async function bootstrapAdmin(args: string[]): Promise<void> {
  ...
  const password = values.password ?? process.env.DSH_USERS_PLATFORM_ADMIN_PASSWORD
  if (username === undefined || username === '' || password === undefined || password === '') {
    console.error('usage: dsh-users-platform bootstrap-admin --username <u> --password <p>')
    process.exit(2)
  }
...
async function runServer(args: string[]): Promise<void> {
  ...
  app.log.info(`dsh-users-platform listening on http://${config.host}:${actualPort}`)
...
async function uidForUserCmd(args: string[]): Promise<void> {
  ...
  if (userId === undefined) {
    console.error('usage: dsh-users-platform uid-for-user <userId> [--db <path>] [--base-uid N]')
    process.exit(2)
  }
