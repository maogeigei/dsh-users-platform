 ...
 * @module dsh-users-platform/supervisor/orchestrator
 */
...
const WATCHDOG_TASK = 'Read DSH_USERS_PLATFORM_HANDOFF_PATH. If it contains a JSON {"command": ...}, run that command. Then exit.'
/* ─────────────────────────────────────────────────────────────────────────────
 ...
 * env 仍然有效，但只承载**其它**选项（如 `--trace-gc`）。要强行指定堆，用 `DSH_USERS_PLATFORM_HEAP_MB_OVERRIDE`。
 */
function withHeap(base: string | undefined, memMb: number): string {
  const override = Number(process.env.DSH_USERS_PLATFORM_HEAP_MB_OVERRIDE ?? '')
...
export class LocalSpawner implements Spawner {
  ...
  constructor(
    ...
    // portal 启动即清掉遗留实例 scope（重启后无法接管）。
  ...
  private async spawnInstance(userId: string, role: InstanceRole, folder: string, patch?: string): Promise<Instance> {
    ...
    const env: Record<string, string> = {
      ...
      DSH_USERS_PLATFORM_ROLE: role,
      DSH_USERS_PLATFORM_HANDOFF_PATH: this.handoffPath(userId), // both roles: main writes, watchdog reads
    }
    if (isMain) {
      env.DSH_USERS_PLATFORM_PORT = String(port)
    }
  ...
  private async spawnAsUser(
    ...
    const bwrapArgs = [
      ...
      // （凭据类 dsh-users-platform.env / shadow / gshadow / sudoers 本就 600/0000，读不到，
      //   本次不涉及；/usr 经审计无任何凭据。）
      //
    ...
    // 回滚：把 384M 改回 512M，npm run build，drain + 重启 dsh-users-platform。
   ...
   *
   * 双通道：① stderr → journald（`journalctl -u dsh-users-platform | grep crash-breaker`）；
