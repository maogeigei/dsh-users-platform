...
OBJECTIVE: 在一台 Linux 机器上装好 **DSH 用户平台**（把 DeepSeek Harness 托管给多个用户），让控制面服务健康并可访问。

DONE WHEN: `systemctl is-active dsh-users-platform` 输出 `active`，且 `curl -fsS -o /dev/null -w '%{http_code}' http://127.0.0.1:${PORT:-3080}/` 输出 `200`。
...
| 现象 | 先看 |
|---|---|
| 服务起不来 | `journalctl -u dsh-users-platform -n 50` |
...
完整说明见 [README.md](README.md)；在仓库里做开发的 agent 请看 [AGENTS.md](AGENTS.md)。
