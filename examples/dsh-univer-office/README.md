...
- The plugin's **worker side still uses TCP** (it assembles a dozen or so URLs with the standard `fetch`; the surface to port is large and the feature is peripheral); **the main flow (exporting a spreadsheet) is unaffected**;
